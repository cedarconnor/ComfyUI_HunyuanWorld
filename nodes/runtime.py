...
class HYWRuntime:
    ...
    def load_image2pano_pipeline(self):
        ...
        if self._image2pano_pipe is None:
            ...
            # Load image pipeline using the same manual component loading approach
            print("Loading FLUX image pipeline components manually...")
            ...
            # Reuse components from text pipeline if already loaded, otherwise load fresh
            if hasattr(self, '_text2pano_pipe') and self._text2pano_pipe is not None:
                print("Reusing components from text pipeline...")
                vae = self._text2pano_pipe.vae
                text_encoder = self._text2pano_pipe.text_encoder
                text_encoder_2 = self._text2pano_pipe.text_encoder_2
                tokenizer = self._text2pano_pipe.tokenizer
                tokenizer_2 = self._text2pano_pipe.tokenizer_2
                scheduler = self._text2pano_pipe.scheduler
            else:
                # Load components fresh (same code as text pipeline)
                from diffusers import FluxTransformer2DModel, AutoencoderKL, FlowMatchEulerDiscreteScheduler
                from transformers import CLIPTextModel, T5EncoderModel, CLIPTokenizer, T5TokenizerFast
                
                print("Loading fresh components for image pipeline...")
                vae = AutoencoderKL.from_single_file(vae_path, torch_dtype=self.torch_dtype, local_files_only=True)
                text_encoder = CLIPTextModel.from_single_file(clip_path, torch_dtype=self.torch_dtype, local_files_only=True)
                text_encoder_2 = T5EncoderModel.from_single_file(t5_path, torch_dtype=self.torch_dtype, local_files_only=True)
                
                try:
                    tokenizer = CLIPTokenizer.from_pretrained("openai/clip-vit-large-patch14", local_files_only=True)
                    tokenizer_2 = T5TokenizerFast.from_pretrained("google/t5-v1_1-xxl", local_files_only=True)
                except:
                    tokenizer = tokenizer_2 = None
                
                scheduler = FlowMatchEulerDiscreteScheduler(
                    num_train_timesteps=1000, shift=1.0, base_image_seq_len=256, max_image_seq_len=4096
                )
            
            # Load FLUX-fill transformer for image pipeline
            print("Loading FLUX-fill transformer...")
            transformer = FluxTransformer2DModel.from_single_file(
                flux_model_path, torch_dtype=self.torch_dtype, local_files_only=True
            )
            
            # Create image pipeline
            self._image2pano_pipe = Image2PanoramaPipelines(
                vae=vae,
                text_encoder=text_encoder,
                text_encoder_2=text_encoder_2,
                tokenizer=tokenizer,
